Shader "Custom/TileShaderBrick"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            float2 scaledTex = IN.uv_MainTex  - float2(floor(IN.uv_MainTex.r*scale) * _MortarSize + floor(IN.uv_MainTex.g * scale)/2, floor(IN.uv_MainTex.g*scale) * _MortarSize);
            fixed4 c = tex2D(_MainTex, scaledTex);
            fixed4 c2 = tex2D(_EdgeTex, IN.uv_EdgeTex) * _Color;

            o.Albedo = c.rgb;
            _ExtrudeVal = 1;
            if (fmod(IN.uv_MainTex.r+_MortarSize+10000/scale+floor(IN.uv_MainTex.g * scale) / 2, 1/scale) < _MortarSize) {
                o.Albedo = c2.rgb;
                _ExtrudeVal = 0;
            }
